...
class Ajax {
    ...
    static #FinalUrl(urlPath, method, data) {
        if(method === 'GET'){
            ...
            throw new Error(`Ajax.#FinalUrl: Method ${method} is not supported for URL building.`);
        }
    }
	...
	static async request(method = 'POST', urlPath, data = null, passedHeaders = {}, timeout = 0, credentials ) {
        ...
        let timerId = null;
        if(finalTimeout > 0){
            timerId = setTimeout(() => controller.abort(`Timeout after ${finalTimeout}ms`), finalTimeout);
        }
	...
	// Convenience methods
	static post(urlPath, data){return this.request('POST', urlPath, data);}
}
